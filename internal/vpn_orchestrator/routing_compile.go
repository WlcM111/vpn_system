...
// (2027-01-01T00:00:00Z). Значение константно и меняться не должно: от него
// откладывается version манифеста.
//
// Точка вынесена в будущее намеренно. Прежняя схема брала метку из mtime файла
// манифеста, то есть отдавала «сейчас». Если новая метка окажется РАНЬШЕ уже
// сохранённой у клиента, Happ не станет перекачивать геофайлы — а профиль
// теперь ссылается на geosite:category-ru, которого в старом кэше может не
// быть, и правила молча перестанут совпадать. Эпоха в будущем гарантирует, что
// первая же выдача после перехода строго позже любого mtime-значения.
const routingGeoEpoch int64 = 1798761600
