...
// subscriptionAnnounce — текст объявления над списком серверов.
//
// Ограничение Happ — 200 символов отображаемого текста; текущий укладывается
// в ~120. Передаётся в base64, потому что HTTP-заголовки ограничены
// ISO-8859-1, а здесь кириллица и эмодзи.
const subscriptionAnnounce = "🤖 Бот: @vpn_house_bot\n" +
	"✉️ Поддержка: support@race-src.com\n" +
	"🚀 Держите HAPP или INCY последней версии"

// subscriptionSupportURL — синяя иконка справа в строке подписки. Для
// ссылок на Telegram Happ рисует фирменную иконку вместо обычной.
const subscriptionSupportURL = "https://t.me/vpn_house_bot"

// subscriptionWebPageURL — иконка слева, «личный кабинет». Без параметра
// иконка остаётся серой и некликабельной.
const subscriptionWebPageURL = "https://t.me/vpn_house_bot"
...
//
// ЕДИНИЦЫ. Двоичные: Happ делит полученные байты на 1024³ и подписывает
// результат как «GB». При десятичном 10^10 на экране получалось 9,31 —
// проверено на живом клиенте. 10 × 1024³ даёт ровно «10 GB».
//
// Расхождение с defaultCDNQuotaLimitBytes = 20_000_000_000 намеренное: там
// величина лимита в байтах, здесь — то, что увидит пользователь.
//
...
const cdnDisplayBytesPerNode int64 = 10_737_418_240
...
func writeSubscriptionHeaders(w http.ResponseWriter, res *SubscriptionFeedResult) {
	...
	// Объявление над списком серверов: контакты и напоминание про версию
	// клиента. Тот же base64, что и у имени, — по той же причине.
	w.Header().Set("announce", encodeProfileTitle(subscriptionAnnounce))
	w.Header().Set("support-url", subscriptionSupportURL)
	w.Header().Set("profile-web-page-url", subscriptionWebPageURL)
	// Запрет на просмотр и редактирование параметров серверов в клиенте.
	// Применяется и к уже добавленным подпискам, и к будущим.
	//
	// Это НЕ шифрование: значения по-прежнему уходят клиенту открытым
	// текстом и видны любому, кто откроет URL подписки в браузере или
	// снимет трафик. Настройка убирает кнопки из интерфейса, не более —
	// см. раздел «Чего это не даёт» в гайде.
